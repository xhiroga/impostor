...
async def read_root():
    ...
    infer_disabled_attr = "disabled" if INFER_ENGINE is None else ""
    infer_notice = (
        f'<p style="color:#f88;margin-top:0.5rem;">{INFER_ENGINE_ERROR}</p>'
        if INFER_ENGINE_ERROR
        else ""
    )
    return HTMLResponse(
        f"""
            ...
            <body>
                <main>
                    ...
                    <section class=\"panel\">
                        ...
                        <p>ImposterのFramePack推論を実行し、生成結果を output ディレクトリに保存します。</p>
                        <form id=\"infer-form\" hx-post=\"/infer\" hx-target=\"#infer-status\" hx-swap=\"innerHTML\" hx-encoding=\"multipart/form-data\" hx-on::after-request=\"window.updateVideoSelect && window.updateVideoSelect()\" style=\"display:flex; gap:0.5rem; align-items:center; flex-wrap:wrap;\">
                            <input type=\"file\" name=\"image\" accept=\"image/*\" required {infer_disabled_attr} style=\"flex:1 1 240px;\" />
                            <button type=\"submit\" style=\"padding:0.5rem 1rem;\" {infer_disabled_attr}>推論スタート</button>
                        </form>
